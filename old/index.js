class TimerWidget {
  ...
  async initWidget() {
    ...
    const timerIssues = await this.dashboardApi.fetch(
      this.youTrack.id,
      `api/issues?query=${query}&fields=idReadable,summary,project(id),customFields(id,name,value(name))&customFields=${activeTimers}&customFields=Timer&$top=1`,
    )
    ...
    this.workItems = await this.dashboardApi.fetch(
      this.youTrack.id,
      `api/admin/projects/${projectId}/timeTrackingSettings/workItemTypes?fields=id,name`,
    )
  ...
  renderTimerDetails() {
    ...
    if (this.timer) {
      const duration = this.timer.endTime
        ? +this.timer.endTime - this.timer.startTime
        : Date.now() - this.timer.startTime
  ...
  async updateTimer() {
    ...
    const response = await this.dashboardApi.fetch(
      this.youTrack.id,
      `api/issues/${issueId}/customFields/${this.timerFieldId}?fields=value(name)`,
      {
        method: "POST",
    ...
    const activeTimers = await this.dashboardApi.fetch(
      this.youTrack.id,
      `api/issues/${issueId}/customFields/${fieldId}?fields=value`,
    )
  ...
  async stopTimer() {
    ...
    await this.dashboardApi.fetch(
      this.youTrack.id,
      `api/issues/${issueId}/customFields/${fieldId}?fields=value(name)`,
      {
        method: "POST",
